// build-kmb-latihan.mjs
// Generator halaman latihan sub-materi Kombinatorika (kmb-xN)
// node tools/build-kmb-latihan.mjs
...
const templatePath = join(ROOT, 'latihan-soal', 'kombinatorika', 'kmb-a1', 'index.html');
const dataPath = join(ROOT, 'content', 'raw', 'kombinatorika.json');
...
const SECTION_JUDUL = {
  ...
  B: 'Permutasi',
  C: 'Kombinasi',
  D: 'Segitiga Pascal & Binomial Newton',
};
...
function buildPage(sectionId, sectionTitle, soalList, paketNum) {
  const kode = `KMB-${sectionId}${paketNum}`;
  ...
  const pageUrl = `${BASE_URL}/latihan-soal/kombinatorika/${paketId}/`;
  ...
  const schema = JSON.stringify({
    ...
    'about': { '@type': 'Thing', 'name': 'Kombinatorika' },
    'hasPart': schemaQuestions,
  });

  let html = template;
  html = html.replace(/const PAKET_ID = 'kmb-a1';/, `const PAKET_ID = '${paketId}';`);
  html = html.replace(/window\.quizDB=\{.*?\};(?=<\/script>)/s, `window.quizDB=${JSON.stringify(quizDB)};`);
  html = html.replace(/KMB-A1/g, kode);
...
for (const section of data.sections) {
  ...
  for (const [num, soalList] of [[1, paket1], [2, paket2]]) {
    const kode = `kmb-${sid.toLowerCase()}${num}`;
    const outDir = join(ROOT, 'latihan-soal', 'kombinatorika', kode);
