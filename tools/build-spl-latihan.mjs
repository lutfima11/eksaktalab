// ─────────────────────────────────────────────────────────────
// build-spl-latihan.mjs
//
// Generator halaman latihan sub-materi Sistem Persamaan Linear (spl-xN)
// dari content/raw/sistem-persamaan-linear.json
//
// Output per section (A-D):
//   latihan-soal/persamaan-linear/spl-x1/index.html  (soal ganjil → campuran)
//   latihan-soal/persamaan-linear/spl-x2/index.html  (soal genap  → campuran)
//
// Pakai:
//   node tools/build-spl-latihan.mjs
// ─────────────────────────────────────────────────────────────
...
const templatePath = join(ROOT, 'latihan-soal', 'persamaan-linear', 'spl-a1', 'index.html');
const dataPath = join(ROOT, 'content', 'raw', 'sistem-persamaan-linear.json');
...
function buildPage(sectionId, sectionTitle, soalList, paketNum) {
  const kode = `SPL-${sectionId}${paketNum}`;
  ...
  const pageUrl = `${BASE_URL}/latihan-soal/persamaan-linear/${paketId}/`;
  ...
  const schema = JSON.stringify({
    ...
    'about': { '@type': 'Thing', 'name': 'Sistem Persamaan Linear' },
  ...
  // PAKET_ID constant
  html = html.replace(/const PAKET_ID = 'spl-a1';/, `const PAKET_ID = '${paketId}';`);
  ...
  // Breadcrumb kode
  html = html.replace(/SPL-A1/g, kode);
...
for (const section of data.sections) {
  ...
  const paket1 = soalAll.filter((_, i) => i % 2 === 0);
  const paket2 = soalAll.filter((_, i) => i % 2 === 1);

  for (const [num, soalList] of [[1, paket1], [2, paket2]]) {
    const kode = `spl-${sid.toLowerCase()}${num}`;
    const outDir = join(ROOT, 'latihan-soal', 'persamaan-linear', kode);
