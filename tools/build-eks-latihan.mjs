// build-eks-latihan.mjs
// Generator halaman latihan sub-materi Eksponen (eks-xN)
// node tools/build-eks-latihan.mjs
...
const templatePath = join(ROOT, 'latihan-soal', 'eksponen', 'eks-a1', 'index.html');
const dataPath = join(ROOT, 'content', 'raw', 'eksponen.json');
...
function buildPage(sectionId, sectionTitle, soalList, paketNum) {
  const kode = `EKS-${sectionId}${paketNum}`;
  ...
  const pageUrl = `${BASE_URL}/latihan-soal/eksponen/${paketId}/`;
  ...
  const schema = JSON.stringify({
    ...
    'about': { '@type': 'Thing', 'name': 'Eksponen' },
    'hasPart': schemaQuestions,
  });

  let html = template;
  html = html.replace(/const PAKET_ID = 'eks-a1';/, `const PAKET_ID = '${paketId}';`);
  html = html.replace(/window\.quizDB=\{.*?\};(?=<\/script>)/s, `window.quizDB=${JSON.stringify(quizDB)};`);
  html = html.replace(/EKS-A1/g, kode);
...
for (const section of data.sections) {
  ...
  for (const [num, soalList] of [[1, paket1], [2, paket2]]) {
    const kode = `eks-${sid.toLowerCase()}${num}`;
    const outDir = join(ROOT, 'latihan-soal', 'eksponen', kode);
