// build-bd-latihan.mjs
// Generator halaman latihan sub-materi Barisan dan Deret (bd-xN)
// node tools/build-bd-latihan.mjs
...
const templatePath = join(ROOT, 'latihan-soal', 'barisan-deret', 'bd-a1', 'index.html');
const dataPath = join(ROOT, 'content', 'raw', 'barisan-deret.json');
...
function buildPage(sectionId, sectionTitle, soalList, paketNum) {
  const kode = `BD-${sectionId}${paketNum}`;
  ...
  const pageUrl = `${BASE_URL}/latihan-soal/barisan-deret/${paketId}/`;
  ...
  const schema = JSON.stringify({
    ...
    'about': { '@type': 'Thing', 'name': 'Barisan dan Deret' },
  ...
  if (html.includes(RENDER_PATCH_FROM)) {
...
for (const section of data.sections) {
  ...
  for (const [num, soalList] of [[1, paket1], [2, paket2]]) {
    const kode = `bd-${sid.toLowerCase()}${num}`;
    const outDir = join(ROOT, 'latihan-soal', 'barisan-deret', kode);
    mkdirSync(outDir, { recursive: true });
    writeFileSync(join(outDir, 'index.html'), buildPage(sid, section.sectionTitle, soalList, num), 'utf8');
    console.log(`✓ ${kode.toUpperCase()} — ${soalList.length} soal`);
    totalPages++;
  }
}
