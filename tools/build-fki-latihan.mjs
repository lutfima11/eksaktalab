// build-fki-latihan.mjs
// Generator halaman latihan sub-materi Fungsi, Komposisi & Invers (fki-xN)
// node tools/build-fki-latihan.mjs
...
const templatePath = join(ROOT, 'latihan-soal', 'fungsi-komposisi-invers', 'fki-a1', 'index.html');
const dataPath = join(ROOT, 'content', 'raw', 'fungsi-komposisi-invers.json');
...
function buildPage(sectionId, sectionTitle, soalList, paketNum) {
  const kode = `FKI-${sectionId}${paketNum}`;
  ...
  const pageUrl = `${BASE_URL}/latihan-soal/fungsi-komposisi-invers/${paketId}/`;
  ...
  const schema = JSON.stringify({
    ...
    'about': { '@type': 'Thing', 'name': 'Fungsi, Komposisi dan Invers' },
    'hasPart': schemaQuestions,
  });

  let html = template;
  html = html.replace(/const PAKET_ID = 'fki-a1';/, `const PAKET_ID = '${paketId}';`);
  html = html.replace(/window\.quizDB=\{.*?\};(?=<\/script>)/s, `window.quizDB=${JSON.stringify(quizDB)};`);
  html = html.replace(/FKI-A1/g, kode);
...
for (const section of data.sections) {
  ...
  for (const [num, soalList] of [[1, paket1], [2, paket2]]) {
    const kode = `fki-${sid.toLowerCase()}${num}`;
    const outDir = join(ROOT, 'latihan-soal', 'fungsi-komposisi-invers', kode);
