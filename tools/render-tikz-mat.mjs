// render-tikz-mat.mjs
// Ekstrak blok TikZ dari .tex Matriks, render ke SVG,
// update matriks.json, siap untuk rebuild quiz pages.
//
// Jalankan: node tools/render-tikz-mat.mjs
...
const SVG_DIR = join(ROOT, 'assets', 'img', 'mat');
const JSON_PATH = join(ROOT, 'content', 'raw', 'matriks.json');
const TEX_DIR = 'D:/14_Database EksaktaLab/Bank Soal/Sub-Materi/Matriks';
const WORK_DIR = join(tmpdir(), 'eksaktalab-mat-tikz');

mkdirSync(SVG_DIR, { recursive: true });
mkdirSync(WORK_DIR, { recursive: true });

function getPreambleExtras() {
  return `\\usetikzlibrary{matrix,positioning,calc,arrows.meta}
\\usepackage{booktabs}
...
function makeStandalone(tikzContent) {
...
const TEX_FILES = {
  F: 'soal_matriks_F.tex',
  G: 'soal_matriks_G.tex',
};

for (const section of data.sections) {
  const sid = section.sectionId;
  const texFileName = TEX_FILES[sid];
  if (!texFileName) continue;

  const texFile = join(TEX_DIR, texFileName);
  ...
  for (const { soalNum, tikzBlock } of tikzBlocks) {
    const label = `mat-${sid.toLowerCase()}${soalNum}`;
    const svgPath = join(SVG_DIR, label + '.svg');

    process.stdout.write(`  Render ${label}... `);

    const standalone = makeStandalone(tikzBlock);
    ...
    if (ok) {
      ...
      if (soal && soal.q.includes('[GAMBAR]')) {
        const relPath = `../../../assets/img/mat/${label}.svg`;
        soal.q = soal.q.replace(
          '[GAMBAR]',
          `<img src="${relPath}" alt="Ilustrasi ${label}" class="soal-gambar" style="display:block;max-width:100%;margin:.5rem auto;">`
        );
      }
    } else {
      totalFailed++;
...
console.log('JSON diupdate. Jalankan: node tools/build-mat-latihan.mjs untuk rebuild quiz pages.');
