// build-fl-latihan.mjs
// Generator halaman latihan sub-materi Fungsi Linear (fl-xN)
// node tools/build-fl-latihan.mjs
...
const templatePath = join(ROOT, 'latihan-soal', 'fungsi-linear', 'fl-a1', 'index.html');
const dataPath = join(ROOT, 'content', 'raw', 'fungsi-linear.json');
...
function buildPage(sectionId, sectionTitle, soalList, paketNum) {
  const kode = `FL-${sectionId}${paketNum}`;
  ...
  const pageUrl = `${BASE_URL}/latihan-soal/fungsi-linear/${paketId}/`;
  ...
  const schema = JSON.stringify({
    ...
    'about': { '@type': 'Thing', 'name': 'Fungsi Linear' },
...
for (const section of data.sections) {
  ...
  for (const [num, soalList] of [[1, paket1], [2, paket2]]) {
    const kode = `fl-${sid.toLowerCase()}${num}`;
    const outDir = join(ROOT, 'latihan-soal', 'fungsi-linear', kode);
