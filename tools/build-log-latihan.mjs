// build-log-latihan.mjs
// Generator halaman latihan sub-materi Logaritma (log-xN)
// node tools/build-log-latihan.mjs
...
const templatePath = join(ROOT, 'latihan-soal', 'logaritma', 'log-a1', 'index.html');
const dataPath = join(ROOT, 'content', 'raw', 'logaritma.json');
...
const SECTION_JUDUL = {
  ...
  C: 'Menyatakan Log dalam Bentuk Lain',
  D: 'Persamaan Logaritma',
  E: 'Pertidaksamaan Logaritma',
  F: 'Fungsi Logaritma',
  G: 'Log Natural dan Perubahan Basis',
  H: 'Aplikasi Logaritma',
};
...
function buildPage(sectionId, sectionTitle, soalList, paketNum) {
  const kode = `LOG-${sectionId}${paketNum}`;
  ...
  const pageUrl = `${BASE_URL}/latihan-soal/logaritma/${paketId}/`;
  ...
  const schema = JSON.stringify({
    ...
    'about': { '@type': 'Thing', 'name': 'Logaritma' },
  ...
  // Tambah/update canonical jika ada
...
for (const section of data.sections) {
  ...
  for (const [num, soalList] of [[1, paket1], [2, paket2]]) {
    const kode = `log-${sid.toLowerCase()}${num}`;
    const outDir = join(ROOT, 'latihan-soal', 'logaritma', kode);
